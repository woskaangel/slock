...
public class MyPageStudentActivity extends AppCompatActivity {
    ...
    TextView userName;
    ImageButton backBtn, deleteNowPwdBtn, deletePwd1Btn, deletePwd2Btn, deleteRoomnumBtn, seeNowBtn, see1Btn, see2Btn;;
    Button continueBtn;
    EditText NowPwd, Roomnum, Pwd1, Pwd2;
    int pwflg = 0;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        deleteNowPwdBtn = (ImageButton)findViewById(R.id.mysNowPwdDel);
        deletePwd1Btn = (ImageButton)findViewById(R.id.mysPwd1Del);
        deletePwd2Btn = (ImageButton)findViewById(R.id.mysPwd2Del);
        deleteRoomnumBtn = (ImageButton)findViewById(R.id.mysRoomnumDel);
        seeNowBtn = (ImageButton)findViewById(R.id.mysNowPwdSee);
        see1Btn = (ImageButton)findViewById(R.id.mysPwd1See);
        see2Btn = (ImageButton)findViewById(R.id.mysPwd2See);

        continueBtn = (Button)findViewById(R.id.mysContinueBtn);

        NowPwd = (EditText)findViewById(R.id.mysNowPwd);
        Roomnum = (EditText)findViewById(R.id.mysRoom);
        Pwd1 = (EditText)findViewById(R.id.mysPwd1);
        Pwd2 = (EditText)findViewById(R.id.mysPwd2);
                ...
                if(pwflg == 1){ // 현재 비밀번호가 일치하는지 확인
                    ...
                    if(!Pwd1.getText().toString().equals("") && !Pwd2.getText().toString().equals("")){ // 바꿀 비밀번호 두개가 비어있지 않은지 확인
                        if(Pwd1.getText().toString().equals(Pwd2.getText().toString())) { // 비밀번호가 두개가 일치하는지 확인
                            ...
                            customDialog = new CustomDialog(MyPageStudentActivity.this, "새로운 비밀번호를 확인해 주세요.");
                            customDialog.show();
                        }
                    }
                    if(!Roomnum.getText().toString().equals("")){
                        ...
                        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, "http://192.168.1.12:8080/rnumchange", new JSONObject(data),
                                new Response.Listener<JSONObject>() {
                                    @Override
                                    public void onResponse(JSONObject response) {
                                        ToastCustom("호실이 변경되었습니다.");
                                        SharedPreference.setAttribute(getApplicationContext(), "roomnumber", Roomnum.getText().toString());
                    ...
                    if(NowPwd.getText().length() == 0){
                        customDialog = new CustomDialog(MyPageStudentActivity.this, "현재 비밀번호를 입력해 주세요.");
                    }
                    else{
                        customDialog = new CustomDialog(MyPageStudentActivity.this, "현재 비밀번호를 확인해 주세요.");
                    }
                    customDialog.show();
                }
            }
